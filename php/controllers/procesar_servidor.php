...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST['generar_qr']) && $_POST['generar_qr']) {
        ...
        // Verificar que el activo existe y es un servidor antes de generar QR
    ...
    if ($accion === "crear") {
        // Obtener configuración de slots para servidor (sin tarjeta de video)
        ...
        // Validar límites de slots para servidor
        if ($slots_cpu < 1 || $slots_cpu > 4) {
            $error = "Cantidad de slots de CPU debe estar entre 1 y 4";
        ...
        if ($slots_ram < 1 || $slots_ram > 16) {
            $error = "Cantidad de slots de RAM debe estar entre 1 y 16";
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                header('Content-Type: application/json');
                echo json_encode(['success' => false, 'error' => $error]);
                exit;
        ...
        if ($slots_almacenamiento < 1 || $slots_almacenamiento > 12) {
            $error = "Cantidad de slots de almacenamiento debe estar entre 1 y 12";
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                header('Content-Type: application/json');
                echo json_encode(['success' => false, 'error' => $error]);
                exit;
        ...
        // MAC
        if (!empty($mac)) {
            $sql_check_mac = "SELECT COUNT(*) as count FROM servidor WHERE mac = ?";
        ...
        // IP
        if (!empty($numeroIP)) {
            $sql_check_ip = "SELECT COUNT(*) as count FROM servidor WHERE numeroIP = ?";
        ...
        if ($id_empresa === null || $id_empresa === '') {
            ...
            error_log("ERROR EMPRESA SERVIDOR: " . $error);
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                header('Content-Type: application/json');
                echo json_encode(['success' => false, 'error' => $error]);
                exit;
        ...
        try {
            // CORREGIDO: Insert servidor con todos los campos necesarios incluyendo link
            $sql_servidor = "INSERT INTO servidor (
            ...
            error_log("SQL INSERT SERVIDOR: " . $sql_servidor);
            error_log("Parámetros SERVIDOR: " . print_r($params_servidor, true));
            ...
            if ($id_servidor === null || $id_servidor === '') {
                ...
                if ($stmt_alt && $row_alt = sqlsrv_fetch_array($stmt_alt, SQLSRV_FETCH_ASSOC)) {
                    ...
                    throw new Exception("El ID del servidor obtenido es nulo o vacío y el método alternativo falló");
                }
            }

            error_log("ID de servidor obtenido: " . $id_servidor);
            ...
            if ($stmt_get_activo_id && $row_activo = sqlsrv_fetch_array($stmt_get_activo_id, SQLSRV_FETCH_ASSOC)) {
                ...
                // Crear slots para servidor (sin tarjeta de video)
                crearSlots($id_activo_nuevo, 'Servidor', $slots_cpu, $slots_ram, $slots_almacenamiento, $conn);
                        ...
                        foreach ($slots_data['rams'] as $ram_data) {
                            if (!empty($ram_data)) {
                                try {
                                    ...
                                    if (strpos($e->getMessage(), 'No hay slots disponibles') !== false) {
                                        error_log("Advertencia SERVIDOR: No hay más slots de RAM disponibles. RAM: $ram_data no asignada.");
                                        break;
                                    } else {
                                        throw $e;
                                    }
                        ...
                        foreach ($slots_data['almacenamientos'] as $almacenamiento_data) {
                            if (!empty($almacenamiento_data)) {
                                try {
                                    ...
                                    if (strpos($e->getMessage(), 'No hay slots disponibles') !== false) {
                                        error_log("Advertencia SERVIDOR: No hay más slots de almacenamiento disponibles. Almacenamiento: $almacenamiento_data no asignado.");
                                        break;
                                    } else {
                                        throw $e;
                                    }
            ...
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                header('Content-Type: application/json');
                echo json_encode(['success' => true, 'message' => 'Servidor creado exitosamente']);
                exit;
            }
            ...
            error_log("Error en transacción SERVIDOR: " . $e->getMessage());
        ...
        $row_serial = sqlsrv_fetch_array($stmt_check_serial, SQLSRV_FETCH_ASSOC);
        if ($row_serial['count'] > 0) {
            $errores_unicidad[] = "❌ Error: El número de serie '$numberSerial' ya existe en otro servidor. Por favor, ingrese un número de serie único.";
        }

        // MAC
        if (!empty($mac)) {
            $sql_check_mac = "SELECT COUNT(*) as count FROM servidor s 
                            INNER JOIN activo a ON s.id_servidor = a.id_servidor 
                            WHERE s.mac = ? AND a.id_activo != ?";
        ...
        // IP
        if (!empty($numeroIP)) {
            $sql_check_ip = "SELECT COUNT(*) as count FROM servidor s 
                            INNER JOIN activo a ON s.id_servidor = a.id_servidor 
                            WHERE s.numeroIP = ? AND a.id_activo != ?";
        ...
        if ($slots_ram < 1 || $slots_ram > 16) {
            die("❌ Error: Cantidad de slots de RAM debe estar entre 1 y 16.");
        }
        if ($slots_almacenamiento < 1 || $slots_almacenamiento > 12) {
            die("❌ Error: Cantidad de slots de almacenamiento debe estar entre 1 y 12.");
        }
        ...
        try {
            ...
            if ($row = sqlsrv_fetch_array($stmt_get_servidor, SQLSRV_FETCH_ASSOC)) {
                ...
                crearSlots($id_activo, 'Servidor', $slots_cpu, $slots_ram, $slots_almacenamiento, $conn);
                ...
                error_log("Servidor editado - Slots actualizados: CPU=$slots_cpu, RAM=$slots_ram, Almacenamiento=$slots_almacenamiento");
            }
            ...
            if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
                header('Content-Type: application/json');
                echo json_encode(['success' => true, 'message' => 'Servidor editado exitosamente']);
                exit;
            }
...
// Función para crear slots de un activo - SERVIDOR (sin tarjeta de video)
...
// Función para asignar componente a slot - SERVIDOR
