...
if($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($stmt) {
        ...
        if (strpos($errors[0]['message'], 'REFERENCE constraint') !== false) {
            header("Location: ../views/crud_almacenamiento_generico.php?error=No se puede eliminar este almacenamiento genérico porque está siendo usado por un activo informático.");
        } else {
            header("Location: ../views/crud_almacenamiento_generico.php?error=Error al eliminar el registro.");
        }
        exit;
    }
}
